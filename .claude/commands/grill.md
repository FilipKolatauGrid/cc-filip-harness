---
description: Decision grill — stress-test design decisions, generate ADRs
phase: planning
reads:
  - "## Design"
...
hard_blocks:
  - condition: "## Design is empty"
    message: "Run `/design` first. Output required in ACTIVE_TASK.md → ## Design."
---
...
After writing all ADRs, append this sentinel as the final line of the `## ADRs` section:

```
<!-- ADRs LOCKED — all decisions above confirmed by developer. Modification requires explicit /grill re-run. Do not edit inline. -->
```

**If you see this sentinel already present:** do not modify any ADR above it. If a decision needs to change, tell the user to re-run `/grill` with the changed constraint.
