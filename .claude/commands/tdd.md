---
description: Test-driven development — write tests per acceptance criterion
...
hard_blocks:
  ...
  - condition: "## Implementation Log is empty AND task type is not bugfix"
...
**Bug-fix mode:** If `## Requirement type = bugfix`, the Implementation Log gate is relaxed. Write the failing test first (reproducing the bug), then run `code` to fix it. This is the correct red-first sequence for bug fixes.
