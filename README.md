...
.claude/
  skills/       task/ init/ design/ grill/ risk/ code/ tdd/ refactor/
                ...
                (each dir has SKILL.md — canonical spec + full skill logic)
  ...
  hooks/        load-context.sh, phase-gate.sh, secops-scan.sh,
                verify-fail-capture.sh, pre-commit.template
...
Five automation hooks enforce discipline at the shell level — no skill changes required.
...
- [x] Hooks (5) — session context, phase gate, secops scan, verify-fail capture, git interlock
- [x] `SKILL_REGISTRY.md`
